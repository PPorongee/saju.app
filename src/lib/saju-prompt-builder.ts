...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData): string[] {
  ...
  prompt += '\n⚠️ 나이 맞춤 해석 규칙 (최우선 규칙!): ' + ageGuideline + '\n' +
    ...
    '- 나이에 맞지 않는 비현실적 예측(60대에 첫 출산, 10대에 부동산 투자 등)은 신뢰를 떨어뜨리니 절대 하지 마.\n\n';
  ...
  prompt += '2027=정미(화/토) 2028=무신(토/금)\n';
  prompt += '[절대규칙: 올해(2026) 월별 운세, 올해 전체 운세 분석은 "올해 운세" 별도 메뉴에서 제공함. 개인사주에서는 2026년 운세를 절대 상세히 다루지 마! "올해는~", "2026년에는~", "병오년~" 식의 올해 운세 서술 금지! 대운/세운 흐름에서 올해를 한 줄 이내로 간략히 언급하는 정도만 OK. 이 규칙을 어기면 전체 응답이 거부됨.]\n';
  ...
  const rules = '=== 규칙 ===\n' +
    '1. 첫 줄부터 바로 ##번호.제목## 형식으로 시작. 인사말 쓰지마. 반드시 ##숫자.제목## 형태의 섹션 마커를 써야 해! 예: ##5.연애 & 인연의 지도##\n' +
    ...
    '10. [중복 금지] 각 섹션은 해당 주제만 다뤄! 다른 섹션에서 이미 다룬 내용을 반복하지 마. 예: 성격 분석은 2번에서만, 연애는 6번에서만, 건강은 8번에서만. 같은 사주 해석을 여러 섹션에서 되풀이하면 탈락!\n' +
    '11. 섹션 끝에 "~하지 않았어?", "~한 적 없어?" 같은 의문문으로 끝내지 마. 단정적이고 자신감 있는 문장으로 마무리해.\n\n' +
    '=== 말투 & 비유 규칙 (가장 중요!!) ===\n' +
    ...
    '핵심: 천간끼리, 지지끼리의 구체적 관계를 찾아서 그 사람만의 이야기를 만들어.\n\n' +
    '=== 심화 사주 개념 필수 사용 규칙 ===\n' +
    '단순 오행 개수만 나열하지 마! 반드시 아래 심화 개념을 활용해:\n' +
    '- 격국(格局): 이 사주의 격국이 뭔지 판별하고, 격국에 맞는 운의 흐름을 설명해. 예: "너는 식신격(食神格)이라 창작/표현으로 먹고사는 구조야"\n' +
    '- 용신(用神)/희신/기신: 이 사주에 가장 필요한 오행(용신)이 뭔지, 도와주는 오행(희신)과 방해하는 오행(기신)이 뭔지 밝혀. 예: "용신이 수(水)니까, 수 기운이 들어오는 시기가 너한테 골든타임이야!"\n' +
    ...
    '=== 말투 & 표현 규칙 (최우선!!) ===\n' +
    '1. 한자를 절대 쓰지 마! 괄호 안에 한자 넣기 금지! 예: 갑목(甲木) → 그냥 "갑목"\n' +
    '2. 고전 문헌 인용 금지! "적천수에~", "자평진전에~", "궁통보감에~" 같은 말 하지 마\n' +
    '3. 어려운 사주 용어는 재밌는 비유로 풀어서 설명해\n' +
    '   예: "편관은 직장 상사처럼 압박을 주는 존재야", "식신은 네 안의 맛집 탐방러 같은 에너지야"\n' +
    '4. 친한 친구한테 조언하듯 다정하고 사근사근하게 말해\n' +
    '5. 비유를 많이 써! 카페, 넷플릭스, 게임, 날씨, 여행 등 일상적인 것에 빗대면 좋아\n\n' +
    '=== 명리학적 근거 필수 규칙 (핵심!!) ===\n' +
    ...
    '좋은 예 (이렇게!):\n' +
    '- "네 재성이 월지에서 힘을 얻고 있어서 기본적으로 돈복이 있는 구조야. 마치 비옥한 땅에 씨를 뿌리면 자연스럽게 열매가 열리는 것처럼~"\n' +
  ...
  let prompt1 = prompt + '=== 아래 5개 항목을 써줘 (1~5번). 6번 이상 쓰지 마! 각 항목을 깊고 풍부하게! ===\n[리마인더: 올해(2026) 운세는 별도 메뉴에서 제공. 여기서는 2026년 운세를 상세히 쓰지 마. ##6## 이상 번호의 섹션을 만들지 마!]\n\n';
  let n = 1;
  ...
  prompt1 += '##' + n++ + '.천직 & 커리어 로드맵## 직장vs사업 적성, 격국에 맞는 업종 2-3개. 숨겨진 재능(식상/인성/편인 기반), 아직 발견 못한 잠재력과 개발법. 시험/합격운(2027~2028). 이직/전직 좋은 시기.\n' +
    '추가로 반드시 포함할 것:\n' +
    '- 🎯 딱 맞는 분야 TOP 5: 일간 오행, 격국, 용신, 식상/재성/관성 구조를 근거로 구체적인 직업/업종을 5개 추천해. 각각 왜 맞는지 사주명리학적 근거를 1-2줄로 설명해.\n' +
    '- ⚠️ 안 맞는 직업 3가지: 기신 오행이나 사주 구조와 충돌하는 업종을 3개 알려줘. 왜 안 맞는지 명리학적 근거도 함께.\n';
  prompt1 += '##' + n++ + '.질문의 답으로 보는 너의 모습 분석## 사용자가 답한 질문들(고민: ' + concernText + ', 현재 상태: ' + stateText + ', 성격: ' + (persText || '미입력') + ', 연애: ' + relText + ')을 사주 기반으로 종합 분석해줘.\n' +
    '- 가장 큰 고민(' + concernText + ')이 사주적으로 왜 생기는지 오행/십성 근거로 설명하고 해결 실마리 제시\n' +
    '- 현재 삶의 상태(' + stateText + ')가 대운/세운 흐름과 어떻게 연결되는지, 일시적인지 흐름의 일부인지\n' +
    '- 자가 성격 진단(' + (persText || '미입력') + ')과 사주에서 보이는 성격 비교 — 일치하면 "역시!", 다르면 "흥미로운데?" 하며 설명\n' +
    '- 현재 연애 상태(' + relText + ')와 사주 연애운 연결 해석\n' +
    '[주의: 올해(2026) 운세를 상세히 다루지 마. 올해 운세가 궁금하면 "올해 운세" 메뉴를 이용해보라고 한 줄 안내만 해.]\n';
  ...
  // Part 2: Relationships, Health, Family (sections 6-11)
  let prompt1b = prompt + '=== 아래 6개 항목을 써줘 (6~11번). 각 항목을 깊고 풍부하게! ===\n[리마인더: 올해(2026) 운세는 별도 메뉴에서 제공. 여기서는 2026년 운세를 상세히 쓰지 마.]\n[출력형식 필수] 반드시 ##번호.제목## 형태로 각 섹션을 시작해! 예: ##6.연애 & 인연의 지도##\n내용...\n##7.나에게 좋은 사람 & 주의할 사람##\n내용...\n이 형식을 안 지키면 전체 응답 거부됨!\n\n';
  if (isMarried) {
  ...
  if (userAge >= 50) {
    prompt1b += '##' + n++ + '.자녀운 & 가족의 미래## 시주와 식상/관성 근거로: 자녀와의 관계(친밀도/갈등 요인), 자녀의 성공·안정 시기, 손주복 여부, 가족 모임이 좋은 시기와 방위. 자녀가 성인이므로 출산 시기 이야기는 하지 마.\n';
  } else {
  ...
  // Part 3: Future predictions (sections 12-14)
  let prompt2 = prompt + '=== 아래 3개 항목을 써줘 (12~14번). 반드시 3개 모두 빠짐없이! ===\n[리마인더: 올해(2026) 운세는 별도 메뉴에서 제공. 여기서는 2026년 운세를 상세히 쓰지 마.]\n[출력형식 필수] 반드시 ##번호.제목## 형태로 각 섹션을 시작해! 예: ##12.향후 10년 미래 시나리오##\n내용...\n##13.결혼 & 인연 타이밍##\n내용...\n##14.내 집 마련 & 부동산##\n내용...\n이 형식을 안 지키면 전체 응답 거부됨!\n\n';
  prompt2 += '##12.향후 10년 미래 시나리오## [필수: 현재는 2026년이야. 2026년 이전(2025, 2024, 2022 등 과거)은 절대 다루지 마! 오직 2027~2036년 미래만!] 향후 10년간의 전체적인 흐름을 연도별로 분석해줘:\n' +
    '- 2027년(정미)부터 상세히 시작. 2027년은 특히 자세하게(상반기/하반기 구분, 천간/지지 합충형, 집중할 영역)\n' +
    '- 2028~2036년까지 매년 세운과 내 원국의 합/충/형 관계를 분석 (과거 연도 절대 금지!)\n' +
    '- 대운 전환 시기와 변화 내용\n' +
    '- 삼재 시기 (삼재 계산: 신자진생→인묘진년, 해묘미생→사오미년, 인오술생→신유술년, 사유축생→해자축년)\n' +
    '- 10년 중 가장 좋은 해 TOP 3 + 조심할 해 TOP 2 (대비법 포함)\n' +
    '- 전체 흐름을 한 문장으로 요약\n';
  if (isMarried) {
    prompt2 += '##13.부부의 미래 & 위기 관리## [중복 금지: 12번에서 다룬 연도별 분석을 반복하지 마!] 부부 관계의 대운 흐름을 "관계" 관점에서만 분석. 배우자궁과 관성/재성의 작용, 부부 사이 좋아지는 조건과 갈등 원인, 극복법과 관계를 깊게 하는 구체적 실천법. 연도별 나열 금지.\n';
  } else {
    prompt2 += '##13.결혼 & 인연 타이밍## [중복 금지: 12번에서 다룬 연도별 분석을 반복하지 마!] 배우자성(정재/정관)이 활성화되는 대운 구간과 조건 분석. 어떤 타입의 인연이 올지(배우자궁 기반), 결혼 적기의 사주적 조건. 연도별 나열 금지.\n';
  }
  prompt2 += '##14.내 집 마련 & 부동산## [중복 금지: 12번에서 다룬 연도별 분석을 반복하지 마!] 사주 원국의 재성/토 기운으로 부동산 체질 분석. ' + (dayOh==='목'?'동':dayOh==='화'?'남':dayOh==='토'?'중앙':dayOh==='금'?'서':'북') + '쪽 유리한 방위와 이유, 맞는 부동산 유형(아파트/상가/토지 등), 재물 축적 전략. 연도별 나열 금지.\n\n';
  prompt2 += getRelevantRefs({ dayMaster: ds, topics: ['timing', 'compatibility', 'general'] });
  prompt2 += rules;

  // Part 4: Lifestyle & closing (sections 15-17)
  let prompt3 = prompt + '=== 아래 3개 항목을 써줘 (15~17번). 반드시 3개 모두 빠짐없이! ===\n[출력형식 필수] 반드시 ##번호.제목## 형태로 각 섹션을 시작해! 예: ##15.행운 루틴 & 개운법##\n내용...\n##16.인생에서 가장 빛나는 나이##\n내용...\n##17.나에게 보내는 편지##\n내용...\n이 형식을 안 지키면 전체 응답 거부됨!\n\n';
  prompt3 += '##15.행운 루틴 & 개운법## 용신 오행에 맞는 매일 행운 습관 3개 + 행운의 색/숫자/방향/요일/계절을 오행 근거와 함께. 기신을 피하는 실생활 팁도.\n';
  prompt3 += '##16.인생에서 가장 빛나는 나이## 대운 흐름과 12운성 변화로 인생 하이라이트 시기 예측. 그때 뭘 하면 좋을지, 이미 지났으면 다음 빛나는 시기.\n';
  prompt3 += '##17.나에게 보내는 편지## 이 사주의 격국, 용신, 대운 흐름을 바탕으로 한 따뜻한 응원 메시지. 감동적이고 진심을 담아서 비유를 많이 써서 가슴에 와닿게.\n\n';
  prompt3 += getRelevantRefs({ dayMaster: ds, topics: ['general'] });
  prompt3 += rules;
  ...
  return [prompt1, prompt1b, prompt2, prompt3];
}
