...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData): string[] {
  ...
  let prompt2 = prompt + '=== 아래 6개 항목을 써줘 (11~16번). 각 항목을 깊고 풍부하게! ===\n\n';
  ...
  if (isMarried) {
    prompt2 += '##12.부부의 미래 & 위기 관리## 부부 관계의 대운 흐름. 부부 사이가 좋아지는 해와 위기가 오는 해를 2026~2036 중에서 구체적으로. 위기 시 극복법과 관계를 깊게 만드는 방법. 재혼운이 있는지도 부드럽게.\n';
  } else {
    prompt2 += '##12.결혼 최적 타이밍## 배우자성(남:재성/여:관성)이 활성화되는 대운/세운 분석. 결혼하기 가장 좋은 해/나이, 어떤 조건에서 결혼하면 행복할지. 통변으로 결혼 사건이 일어나는 세운 지지 계산.\n';
  }
  prompt2 += '##13.내 집 마련 & 부동산## 부동산 시기(토/금 대운). ' + (dayOh==='목'?'동':dayOh==='화'?'남':dayOh==='토'?'중앙':dayOh==='금'?'서':'북') + '쪽 유리. 묘(墓)와 재성의 관계로 재물 축적 시기 분석.\n';
  prompt2 += '##14.행운 루틴 & 개운법## 용신 오행에 맞는 매일 행운 습관 3개 + 행운의 색/숫자/방향/요일/계절을 오행 근거와 함께. 기신을 피하는 실생활 팁도.\n';
  prompt2 += '##15.인생에서 가장 빛나는 나이## 대운 흐름과 12운성 변화로 인생 하이라이트 시기 예측. 그때 뭘 하면 좋을지, 이미 지났으면 다음 빛나는 시기. 용신 대운과 건록/제왕이 겹치는 시기 탐색.\n';
  prompt2 += '##16.나에게 보내는 편지## 이 사주의 격국, 용신, 대운 흐름을 바탕으로 한 따뜻한 응원 메시지. "너의 사주를 보니 이런 사람이고, 이런 미래가 기다리고 있어"라는 느낌으로 감동적이고 진심을 담아서. 비유를 많이 써서 가슴에 와닿게.\n\n';
