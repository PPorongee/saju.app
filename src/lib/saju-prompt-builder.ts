...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData): string[] {
  ...
  // Part 3: Future predictions (sections 11-13)
  let prompt2 = prompt + '=== 아래 3개 항목을 써줘 (11~13번). 반드시 3개 모두 빠짐없이! ===\n\n';
  prompt2 += '##11.향후 10년 미래 시나리오 (2027~2036)## [주의: 2026년 운세는 "올해 운세" 별도 메뉴에서 제공. 여기서 2026년은 한줄 요약만 쓰고 2027년부터 상세히.] 향후 10년간의 전체적인 흐름을 연도별로 분석해줘:\n' +
    ...
    '- 전체 흐름을 한 문장으로 요약\n';
  if (isMarried) {
    prompt2 += '##12.부부의 미래 & 위기 관리## 부부 관계의 대운 흐름. 좋아지는 해와 위기의 해를 구체적으로. 극복법과 관계 깊게 하는 방법.\n';
  } else {
    prompt2 += '##12.결혼 최적 타이밍## 배우자성이 활성화되는 대운/세운 분석. 결혼하기 좋은 해/나이.\n';
  }
  prompt2 += '##13.내 집 마련 & 부동산## 부동산 시기(토/금 대운). ' + (dayOh==='목'?'동':dayOh==='화'?'남':dayOh==='토'?'중앙':dayOh==='금'?'서':'북') + '쪽 유리. 재물 축적 시기 분석.\n\n';
  prompt2 += getRelevantRefs({ dayMaster: ds, topics: ['timing', 'compatibility', 'general'] });
  prompt2 += rules;

  // Part 4: Lifestyle & closing (sections 14-16)
  let prompt3 = prompt + '=== 아래 3개 항목을 써줘 (14~16번). 반드시 3개 모두 빠짐없이! ===\n\n';
  prompt3 += '##14.행운 루틴 & 개운법## 용신 오행에 맞는 매일 행운 습관 3개 + 행운의 색/숫자/방향/요일/계절을 오행 근거와 함께. 기신을 피하는 실생활 팁도.\n';
  prompt3 += '##15.인생에서 가장 빛나는 나이## 대운 흐름과 12운성 변화로 인생 하이라이트 시기 예측. 그때 뭘 하면 좋을지, 이미 지났으면 다음 빛나는 시기.\n';
  prompt3 += '##16.나에게 보내는 편지## 이 사주의 격국, 용신, 대운 흐름을 바탕으로 한 따뜻한 응원 메시지. 감동적이고 진심을 담아서 비유를 많이 써서 가슴에 와닿게.\n\n';
