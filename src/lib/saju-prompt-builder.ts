...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData, cachedYongsin?: YongsinMeta): string[] {
  ...
  const referenceQualitySeed =
    ...
    (userShadow ? '  [약점=강점 그림자]\n' +
      '    강점: ' + userShadow.strength + '\n' +
      '    그림자: ' + userShadow.shadow + '\n' +
      '    한 줄 처방: ' + userShadow.remedy + '\n' +
      '    → ##1 문단 4(약점 직격)에서 "이런 기운을 가진 분들은 [강점]의 아이콘이지만, [그림자]" 식으로 풀어줘.\n' +
      '    → ##1 문단 5(마지막 한 줄 격언)에서 위 처방을 짧고 강하게.\n' +
      '    ★ 이 격언 문장은 ##1 마지막에만. ##2·##3·##4는 그 섹션 특화 격언(돈→재물 처방, 연애→인연 처방)으로 마무리. 동일 문장 반복 X.\n' : '') +
    '\n';

  // ============================================================
  // Part 1: sections 1~4
  // ============================================================
  // ★★★ 강제 헤더 — 가장 자주 무시되는 핵심 룰만. 무게 다이어트 (R15+).
  const hardForcedHeader =
    '🚨 절대 위반 금지 (어기면 응답 거부) 🚨\n\n' +
    '[룰 1] 응답 첫 줄: [용신: X / 기신: Y / 희신: Z / 근거: 명리 라벨 / 풀이: 친근한 2~3문장] — 5필드 모두 채움.\n\n' +
    '[룰 2] 이 사주 신살: ' + shinsalStr + '\n' +
    ...
    '  "신살" 단어 X. 이름만 또는 "별".\n' +
    forcedShinsalBlock + '\n\n' +
    '[룰 3] 응답은 정확히 ##1, ##2, ##3, ##4 4개 섹션. ##5+ 금지.\n' +
    '  ##2 인생 로드맵 5단계는 ##2 본문 내 텍스트 라벨로만 — ##기호 X.\n' +
    '  ##3 = 돈/커리어. ##4 = 연애/부부. 다른 주제로 ##3·##4 사용 X.\n\n' +
    '★ 위 3룰은 본문 모든 룰을 압도. ★\n\n';

  let prompt1 = hardForcedHeader + referenceQualitySeed + compactRules + yongsinHeaderRule + prompt + '=== 아래 4개 항목을 써줘 (1~4번). 5번 이상 쓰지 마! 각 항목은 "이 사주만의 디테일"로 ===\n[리마인더] 올해(2026) 운세는 별도 메뉴에서 제공. 여기서는 2026년 운세를 상세히 다루지 마.\n[리마인더] 일반론·당연한말 금지. 각 섹션은 5~6문단 권장.\n\n';
  let n = 1;
