...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData): string[] {
  ...
  prompt2 += '##12.향후 10년 미래 시나리오## [필수: 현재는 2026년이야. 2026년 이전(2025, 2024, 2022 등 과거)은 절대 다루지 마! 오직 2027~2036년 미래만!] 향후 10년간의 전체적인 흐름을 연도별로 분석해줘:\n' +
    ...
    '- 전체 흐름을 한 문장으로 요약\n';
  if (isMarried) {
    prompt2 += '##13.부부의 미래 & 위기 관리## 부부 관계의 대운 흐름. 좋아지는 해와 위기의 해를 구체적으로. 극복법과 관계 깊게 하는 방법.\n';
  } else {
    prompt2 += '##13.결혼 & 인연 타이밍## 배우자성이 활성화되는 대운/세운 분석. 결혼하기 좋은 해/나이.\n';
  }
  prompt2 += '##14.내 집 마련 & 부동산## 부동산 시기(토/금 대운). ' + (dayOh==='목'?'동':dayOh==='화'?'남':dayOh==='토'?'중앙':dayOh==='금'?'서':'북') + '쪽 유리. 재물 축적 시기 분석.\n\n';
