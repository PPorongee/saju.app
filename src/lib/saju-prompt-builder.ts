...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData): string[] {
  ...
  const rules = '=== 규칙 ===\n' +
    ...
    '=== 솔직한 단점/약점 분석 규칙 (신뢰도 핵심!) ===\n' +
    '각 섹션에서 일간의 성격적 단점과 그로 인한 리스크를 솔직하게 짚어줘. 이게 있어야 "내 얘기네!" 소름이 돋음:\n' +
    '- 성격 섹션: 강점만 나열 금지! 일간의 고질적 약점 2가지를 콕 찍어. 예: "경금 일간이라 완벽주의가 심해서 사소한 일에도 스트레스를 받고, 남한테 맡기는 걸 못해서 혼자 다 끌어안다 번아웃이 올 수 있어"\n' +
    '- 재물 섹션: "이 성격 때문에 돈을 이렇게 잃기 쉬워" 식의 성격-재물 연결. 예: "식상이 강해서 기분 좋으면 지갑이 열리는 타입. 친구들한테 밥 사주다가 통장이 텅 비는 패턴 조심"\n' +
    '- 연애 섹션: "이런 성격 때문에 연애에서 이런 실수를 반복해" 패턴 분석\n' +
    '- 직장 섹션: "이 성격이 직장에서 이런 갈등을 만들어" 예측\n' +
    '- 건강 섹션: "이 성격/생활 패턴이 이런 건강 문제로 이어질 수 있어" 연결\n' +
    '단, 모든 약점 지적 뒤에는 반드시 "그래서 이렇게 하면 돼" 식의 구체적 해결책을 붙여!\n' +
    '비율: 강점 이야기 후 → 단점/리스크 → 해결책 순서로 자연스럽게 이어가\n\n' +
    '=== 말투 & 비유 규칙 (가장 중��!!) ===\n' +
