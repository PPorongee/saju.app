...
export function buildSajuPrompts(sj: SajuResult, ohCount: Record<string, number>, userData: UserData): string[] {
  ...
  let prompt1 = prompt + '=== 아래 4개 항목만 써줘 (1~4번). 5번 이상 쓰지 마! 각 항목을 깊고 풍부하게! ===\n[리마인더: 올해(2026) 운세는 별도 메뉴에서 제공. 여기서는 2026년 운세를 상세히 쓰지 마. ##5## 이상 번호의 섹션을 만들지 마!]\n\n';
  let n = 1;
  ...
  // Part 3: Future predictions (sections 11-14)
  let prompt2 = prompt + '=== 아래 4개 항목을 써줘 (11~14번). 반드시 4개 모두 빠짐없이! ===\n[리마인더: 올해(2026) 운세는 별도 메뉴에서 제공. 여기서는 2026년 운세를 상세히 쓰지 마.]\n\n';
  prompt2 += '##11.2027년 미리보기## 내년(2027 정미년)의 세운이 내 원국과 어떻게 작용하는지. 천간/지지 합충형 분석, 주요 변화 포인트, 상반기/하반기 흐름, 집중할 영역과 조심할 점.\n';
  prompt2 += '##12.향후 10년 미래 시나리오 (2027~2036)## 향후 10년간의 전체적인 흐름을 연도별로 분석해줘:\n' +
    '- 2027~2036년까지 매년 세운과 내 원국의 합/충/형 관계를 분석\n' +
    '- 대운 전환 시기와 변화 내용\n' +
  ...
  if (isMarried) {
    ...
    prompt2 += '##13.결혼 최적 타이밍## 배우자성이 활성화되는 대운/세운 분석. 결혼하기 좋은 해/나이.\n';
  }
