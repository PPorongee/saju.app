...
const EVIDENCE_TO_NARRATIVE_BLOCK = [
  ...
  '1) 명리 근거 — 핵심 용어(일지·충·합·용신·십성 등)를 한 번 가볍게 드러낸다. 완전히 지우지 말 것.',
  '   (사주 근거가 보여야 "이 사주만의 궁합"으로 읽힌다. 단 용어를 나열·반복하지는 않는다.)',
  '2) 쉬운 뜻 — 그 용어를 바로 그 자리에서 일상어로 풀이 (용어:풀이 ≈ 2:8).',
  '3) 두 사람 사이에서 그것이 어떤 방식으로 나타나는지 (관계 장면)',
  '4) 좋게 작동할 때 (활성화 방향)',
  '5) 꼬일 수 있을 때 (회피·주의 방향)',
  '6) 관계를 좋게 쓰는 사용법 (행동/거리/역할/타이밍 — 부적·색상·방향 같은 미신 X)',
  '',
  '[나쁜 예 — 단정] "두 사람은 일지에 충이 있어 궁합이 나쁩니다." (X: "나쁘다"는 단정)',
  '[나쁜 예 — 용어 완전 삭제] "가장 가까운 자리에서 끌리면서도 흔들립니다." (X: 무슨 근거인지 안 보여 일반 연애조언처럼 읽힘)',
  '[좋은 방향 — 용어 1번 + 즉시 풀이 + 단정 없음]',
  '  "두 사람의 가장 가까운 자리(일지)가 서로 강하게 끌어당기면서도 부딪히는 결(충)이라, ',
  '   가까워질수록 같은 자리에서 끌림과 마찰이 같이 올라오기 쉬워요. 거리를 어떻게 조절할지 미리 정해두면 편해집니다."',
].join('\n');
...
const SECTION_OUTPUT_RULES: Record<CompatNarrativeSectionId, string[]> = {
  ...
  relationshipMechanism: [
    ...
    '★ 핵심 용어를 최소 1~2개는 그대로 한 번 드러낸 뒤 바로 쉬운 말로 푼다(예: "일지(가장 가까운 자리)가 …", "상대의 ○ 기운이 내 용신이라 …"). 용어를 100% 일상어로 지우지 말 것 — 사주 근거가 보여야 한다.',
