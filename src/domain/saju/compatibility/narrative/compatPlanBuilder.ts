...
function buildMechanismPlan(facts: CompatMustUseFact[], ctx: CompatNarrativeContext): CompatNarrativePlan {
  return {
    ...
    requiredBeats: [
      ...
      '십성 작용 — 상대가 나에게 어떤 역할로 느껴지는지 (용어 즉시 풀이).',
      '용신/기신 상호 자극 — 편한 기운인지 부담스러운 기운인지.',
      '일지(가장 가까운 자리)에서 어떻게 맞물리는지.',
    ...
    styleExamples: {
      ...
      goodExample:
        '두 사람의 기본 기운은 한쪽이 주는 결이 조금 더 강해서, 한 사람이 분위기를 끌고 가는 흐름이 자연스럽게 생깁니다. ' +
        '가장 가까운 자리에서는 서로를 강하게 끌어당기는 동시에 생활 리듬이 흔들리는 결이 함께 있어서, 가까워질수록 같은 자리에서 끌림과 부딪힘이 같이 올라옵니다. ' +
        '그래서 이 관계는 거리를 잘 조절할수록 편해지는 구조입니다.',
      transformationRule: '명리 근거 → 쉬운 뜻 → 두 사람 사이의 방식 흐름으로. 합충 나열 금지.',
    },
