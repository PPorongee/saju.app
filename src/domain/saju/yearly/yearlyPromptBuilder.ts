...
const YEARLY_TONE_RULES_BLOCK = [
  ...
  '명리 근거(세운 간지·십성, 대운, 용신/기신 관계, 합·충, 신살 등)는 별도 목록으로 나열하지 말고, 각 해석 문장 안에 "왜 그런 흐름인지"로 자연스럽게 녹여라. 근거가 곧 신뢰이므로 빼먹지 말되, 어려운 용어는 그 자리에서 바로 쉬운 말로 풀어 설명한다. (예: "세운 천간이 일간을 누르는 결이라" → "올해 들어오는 기운이 나를 시험하듯 눌러서")',
].join('\n');
...
const EVIDENCE_TO_NARRATIVE_BLOCK = [
  ...
  '  "올해 재성이 들어와 돈운이 있습니다."',
  '[좋은 방향]',
  '  "올해 재성 흐름은 돈 자체만 뜻하기보다, 현실 조건·보상 기준·사람과의 약속이 중요해지는 흐름으로 볼 수 있습니다. ',
  '   그래서 올해는 큰 한 방을 기대하기보다, 내가 맡는 일의 범위와 보상 기준을 명확히 정할 때 운이 안정적으로 살아납니다."',
].join('\n');
...
const JSON_SCHEMAS: Record<YearlyFortuneSectionId, string> = {
  ...
  topicFortunes: [
    ...
    '  "topicFortunes": {',
    '    "career": string,        // 일·커리어 (단정 X)',
    '    "money": string,         // 돈·수익 (투자 조언 0)',
    '    "relationship": string,  // 관계·연애 (relationshipStatus 정책 준수)',
    '    "rhythm": string         // 생활 리듬·멘탈 (의학 진단 0)',
    '  }',
    '}',
  ].join('\n'),
...
const SECTION_OUTPUT_RULES: Record<YearlyFortuneSectionId, string[]> = {
  ...
  topicFortunes: [
    ...
    'rhythm: 질병/진단/치료/약물/발병/중병 단어 0건. 생활 리듬·수면·휴식·생각 정리 중심.',
