...
const YEARLY_TONE_RULES_BLOCK = [
  ...
  '★ [명리 근거 살짝 드러내기] 핵심 용어 2~3개(대운·용신/기신·신강신약·세운 십성 중)는 그 용어를 한 번 그대로 드러내고 바로 쉬운 말로 풀어라. 용어를 100% 일상어로 지우지 마라 — 사주 근거가 보여야 "이 사주만의 올해운세"가 된다. (예: "지금 거쳐가는 큰 시기(대운)가 …", "곁에 있으면 한결 편해지는 기운(용신)이 …")',
  '🚫 [용어 나열·덤프 금지] 단, 용어를 줄줄이 늘어놓지 마라: "대운 병술·편관·본기 편인·오술 반합·신강 구조"처럼 술어를 나열하거나, 간지 이름(병술·갑오·을미 등)·지장간 본기/여기/중기·합충형파해 종류를 그대로 나열하면 실패다. 한 문장에 새 용어는 하나까지, 반드시 즉시 풀이를 붙인다.',
  '변환 예시(용어 1개 드러내고 즉시 풀이): "현재 대운 병술이 천간에서 편관의 기운을 준다" → "지금 거쳐가는 큰 시기(대운)가, 나를 시험하듯 책임을 무겁게 지우는 환경이에요"; "올해 화 기운은 중립적" → "올해 새로 들어오는 기운은 크게 밀거나 당기지 않고 지금 흐름을 그대로 이어가는 편이에요"; "신강 구조라 지원적" → "원래 에너지가 단단한 편(신강)이라 이 흐름을 무리 없이 받쳐줘요". 나열 금지 예: "오술 반합·편인 본기" → 풀어서 "여러 일이 한 방향으로 모이고, 그 안쪽엔 혼자 파고들어 배우려는 결도 함께 흐르는 흐름".',
].join('\n');
...
const EVIDENCE_TO_NARRATIVE_BLOCK = [
  ...
  '[나쁜 예 — 용어 나열/단정]',
  '  "올해 재성이 들어와 돈운이 있습니다."',
  '[좋은 방향] (핵심 용어 1개를 드러내고 즉시 풀이 → 현실 → 전략)',
  '  "올해 들어오는 기운(세운)은 돈 자체보다, 현실적인 조건과 보상 기준·사람과의 약속이 중요해지는 결로 볼 수 있습니다. ',
...
function renderFact(f: YearlyMustUseFact): string {
  const lines: string[] = [];
  lines.push(`  - [${f.id} / ${f.source}] 근거 fact(나열·덤프 금지. 핵심 용어 하나는 한 번 드러내되 즉시 풀이): "${f.fact}"`);
  lines.push(`      ✅ 본문에는 이 "쉬운 풀이"를 중심으로 녹이되, 핵심 용어는 한 번 드러내고 바로 풀어라: ${f.plainMeaning}`);
