...
규칙(절대):
1. 친근한 존댓말(해요체)로 쓴다. 반말 절대 금지(리포트의 다른 본문과 톤을 통일). 사주 상담 톤(예: "~한 결이에요", "~할 때 풀려요")으로 2~5문장의 매끄러운 한 단락으로 다듬는다. 어미는 "~이에요/~한 편이에요/~할 때 풀려요/~로 볼 수 있어요/~는 게 좋아요"처럼 다양하게 섞는다. 나열 대시(—)·세미콜론을 풀어 문장으로 연결한다.
2. "(근거: …)" 괄호는 그대로 두지 말고, 그 내용을 문장 안에 녹여라. "왜 그런지"가 자연스럽게 읽혀야 신뢰감이 생긴다. 예: "위기 때 손 내밀어주는 윗사람을 가까이 두면 좋아요. 결정적인 순간에 사람 덕을 보는 기운이 있거든요."
