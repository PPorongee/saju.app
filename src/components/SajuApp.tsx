...
export default function SajuApp() {
  ...
  async function fetchYearlyReading(sj: SajuResult, signal?: AbortSignal) {
    ...
    const sectionsP1 =
      '=== 이 파트에서는 1번, 2번 섹션만 써! 3번 이후는 절대 쓰지 마! ===\n' +
      ...
      '최소 35줄. 비유 풍부하게! 올해의 테마를 충분히 깊이 있게 풀어줘!\n\n' +
      ...
      '🛑 여기서 멈춰! ##1과 ##2만 쓰고 끝내! ##3 이후 섹션은 절대 쓰지 마!\n\n' +
    ...
    const sectionsP2 =
      '=== 아래 4개 항목을 써줘 (3~6번). 반드시 4개 모두 빠짐없이! ===\n' +
      '[중복 금지] 이전 파트(1~2번)에서 이미 다룬 내용을 절대 반복하지 마! 월별 운세에서 한 이야기를 다시 쓰면 탈락.\n' +
      ...
      '- 집중할 영역 vs 쉬어야 할 영역\n' +
      '- 비유로 표현 (예: "3분기는 마라톤의 언덕 구간")\n' +
      '- ⚠️ 에너지 낮은 분기는 왜 힘든지, 성격상 어떤 실수를 저지르기 쉬운지 직구로 써!\n' +
      '최소 25줄. 각 분기를 깊이 있게 분석해!\n\n' +
      ...
      '- 구체적 금액 예측 금지, 흐름과 타이밍만\n' +
      '- ⚠️ 돈과 관련된 성격 약점도 솔직하게! "너는 이런 성격이라 이런 식으로 돈을 날릴 수 있어" 식 경고 필수\n' +
      '최소 20줄. 재물 흐름을 월별로 구체적으로!\n\n' +

      '##5.올해 연애운 & 인연 캘린더##\n' +
      '[⚠️ 중복 금지] 섹션2에서 연애 이야기를 안 썼으니 여기서 처음 다루는 것처럼 깊이 있게. 섹션7(대인관계)과는 로맨스 vs 사회적 인연으로 완전 구분.\n' +
      '① 올해 연애 체질 진단: 도화살/홍염살/배우자성 + 병오년 자극 분석 (5줄)\n' +
      '② 인연 키워드: 잘 맞는 상대 오행/띠/성격 (3줄)\n' +
      '③ 연애 에너지 TOP 3 시기: 핵심 3개월만 (월별 전체 나열 금지!)\n' +
      '④ 주의할 감정 시기: 1~2개월 + 대처법\n' +
      '⑤ 솔로 vs 커플 맞춤 조언\n' +
      '- ⚠️ 연애에서 반복하는 실수 패턴도 솔직하게 짚어줘! "너는 연애할 때 이런 성격 때문에 이런 문제가 반복돼" 식 팩트폭행\n' +
      '명리학적 근거 필수! 최소 30줄. 연애운 섹션은 풍성하게!\n\n' +
      ...
      '- ⚠️ 직장에서 성격 때문에 손해보는 패턴도 직구로! "너는 이런 상황에서 이렇게 반응해서 손해봐"\n' +
      '[건강]\n' +
      '- 병오년 화(火) 기운이 오행 균형에 미치는 건강 영향\n' +
      '- 월별 컨디션 변화, 취약 장기 관리법, 맞는 운동/음식\n' +
      '- ⚠️ 건강 관련 나쁜 습관도 지적해줘! "너는 이런 체질이라 이런 걸 계속하면 진짜 위험해"\n' +
      '[끝에 "※ 명리학적 관점의 건강 경향성이며 의학적 조언이 아닙니다" 포함]\n' +
      '최소 25줄. 직장과 건강 각각 깊이 있게!\n\n' +
    ...
    const promptPart2 = promptBase + sectionsP2;

    const prompt3Yearly = promptBase +
      '=== 아래 4개 항목을 써줘 (7~10번). 반드시 4개 모두 빠짐없이! ===\n' +
      '[중복 금지] 이전 파트(1~6번)에서 이미 다룬 내용을 절대 반복하지 마! 특히 월별 운세/재물운/연애운/직장건강에서 한 이야기를 다시 쓰면 탈락.\n' +
      '[⚠️ "마치" 단어 사용 절대 금지! 한 번이라도 쓰면 탈락!]\n' +
      '[명리학 근거 필수] 모든 해석에 "왜?"를 명리학적으로 설명해.\n' +
      '[출력형식 필수] 반드시 ##번호.제목## 형태로 각 섹션을 시작해!\n' +
      '[팩트폭행 리마인더] 모든 섹션에서 좋은 흐름 55% + 솔직한 경고/단점 25% + 해결책 20% 비율 지켜! 칭찬만 가득한 섹션은 탈락!\n\n' +
      ...
      '최소 25줄. 감동적이고 비유 풍부하게, 올해를 완벽하게 마무리하는 에세이처럼!\n\n' +
