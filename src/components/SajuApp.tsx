...
export default function SajuApp() {
  ...
  async function fetchYearlyReading(sj: SajuResult, signal?: AbortSignal) {
    ...
    const sectionsP1 =
      '=== 아래 5개 항목(1~5번)을 순서대로 써줘. 반드시 5개 모두 빠짐없이! 6번 이후는 절대 쓰지 마! ===\n' +
    ...
    const prompt2Yearly = promptBase +
      '=== 아래 5개 항목을 써줘 (6~10번). 반드시 5개 모두 빠짐없이! ===\n' +
      '[중복 금지] 이전 파트(1~5번)에서 이미 다룬 내용을 절대 반복하지 마! 특히 월별 운세/재물운/연애운에서 한 이야기를 다시 쓰면 탈락.\n' +
    ...
    const expectedSectionNums = [
      [1, 2, 3, 4, 5],       // Part 1: sections 1-5
      [6, 7, 8, 9, 10],      // Part 2: sections 6-10
    ];
