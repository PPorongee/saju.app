...
export default function SajuApp() {
  ...
  async function fetchYearlyReading(sj: SajuResult, signal?: AbortSignal) {
    ...
    const promptBase = (lang === 'en' ? '🚨 CRITICAL LANGUAGE INSTRUCTION 🚨\nYou MUST write EVERYTHING in English. Translate Korean section titles to English.\nSaju terms like Gap(甲) can appear with English meaning, but ALL text must be English.\nIF YOU WRITE IN KOREAN, THE RESPONSE WILL BE REJECTED.\n\n' : '') +
      ...
      '이 재미 요소들은 올해운세 전체 17개 섹션 모두에 적용해!\n\n' +
      '【분석 대상 - 전체 사주 원국】\n' +
      ...
      '- 섹션9(연애)와 섹션13(대인관계)은 로맨스 vs 사회적 인연으로 완전 구분. 섹션9에서 쓴 월/시기를 섹션13에서 반복 금지\n' +
      '- 섹션2(월별)에서 연애 이야기 금지! 연애는 오직 섹션9에서만 다뤄\n' +
    ...
    const sectionsP1 =
      ...
      '- ⚠️ 긍정적인 내용만 쓰지 마! 각 월마다 조심해야 할 것, 힘들 수 있는 부분, 안 좋은 흐름도 반드시 포함해. 좋은 점과 주의할 점의 비율을 6:4 정도로 균형 맞춰.\n' +
      '- 이 사람의 일간 성격 특성을 고려해서 "이런 성격이라 이 달에 특히 힘들 수 있어" 같은 맞춤 경고를 넣어 (예: 일간이 편관 기운이 강한 달에 참을성이 부족한 사람이면 충동적 결정 주의)\n' +
      '- 충/형/파가 있는 달은 구체적으로 어떤 갈등/손실/건강 문제가 생길 수 있는지 솔직하게 알려줘. 무조건 "괜찮아~" 하지 마!\n' +
      ...
      '- 비유 퀄리티: 뻔한 비유 금지! "씨앗을 뿌리는 시기" "꽃이 피는" 같은 식상한 비유 쓰지 마. 대신 구체적이고 생생한 비유를 써: "주식 상한가 치는 느낌", "레벨업 직전 경험치 몰아주는 구간", "보스전 앞 세이브포인트", "복권 당첨은 아닌데 로또 3개 맞은 기분", "면접관이 네 이력서 보고 미소 짓는 순간" 같은 현실적이고 와닿는 비유!\n' +
      '- 비유 영역: 게임/주식/쇼핑/운동/드라마/영화/SNS/직장생활 등 현대적 소재에서 가져와. 자연 비유만 반복하면 탈락\n' +
      '- 구체적 장면 묘사: "좋은 달이야" (X) → "회의실에서 네 아이디어가 빛을 발하는 순간이 올 거야" (O)\n' +
      '- 운의 강도 표현: 좋은 흐름의 강도를 체감되게 알려줘! 살짝 좋은 정도면 "소소하게 운이 따르는 정도", 꽤 좋으면 "이건 진짜 잘 타고 있는 흐름이야!", 아주 좋으면 "대박 예감🔥 올해 이 달이 터닝포인트야!", "이건 인생에서 몇 번 안 오는 타이밍이야" 같은 강도 차이를 분명히 해\n' +
      '- 안 좋은 흐름도 강도 표현: "약간 찝찝한 정도" vs "진짜 조심해야 해, 이건 빨간불이야🚨" vs "이 달은 좀 숨 고르기 하면서 버텨야 해" 등 강도를 구분\n' +
      ...
      '##3.올해 분기별 에너지 리듬##\n' +
      '1분기(1~3월), 2분기(4~6월), 3분기(7~9월), 4분기(10~12월)의 에너지 수준을 각각 10점 만점으로 점수를 매겨줘.\n' +
      '반드시 다음 형식으로 첫 줄에 써: [에너지점수: Q1=X, Q2=X, Q3=X, Q4=X]\n' +
      '각 분기별로:\n' +
      '- 해당 분기의 월운 천간지지가 내 원국과 어떤 관계를 형성하는지 (합/충/형 구체적으로)\n' +
      '- 에너지가 높은 이유 또는 낮은 이유를 명리학적 근거로 설명\n' +
      '- 그 분기에 집중해야 할 영역과 쉬어야 할 영역\n' +
      '- 각 분기를 비유로 표현 (예: "3분기는 마라톤의 언덕 구간이야")\n' +
      '최소 12줄.\n\n' +
      ...
      '##9.올해 연애운 & 인연 캘린더##\n' +
      '[⚠️ 중복 금지 핵심] 섹션2(월별 운세)에서는 연애 이야기를 일절 안 썼어. 그러니 여기서 연애를 처음 다루는 것처럼 깊이 있게 써줘. 단, 섹션13(대인관계)과는 구분해: 여기는 "로맨스/연인/배우자" 전용이고, 섹션13은 "친구/동료/귀인" 전용이야.\n' +
      '다음 구조로 써줘:\n' +
      '① 올해 연애 체질 진단: 내 원국의 도화살/홍염살/배우자성(정재 or 정관) 상태 + 병오년 세운이 이걸 어떻게 자극하는지 명리적 분석 (5줄 이상)\n' +
      '② 인연 키워드: 나와 잘 맞는 상대의 오행/띠/성격 유형을 사주 근거로 (3줄)\n' +
      '③ 연애 에너지 TOP 3 시기: 12개월 중 로맨스 에너지가 가장 강한 달 3개만 뽑아서, 각각 왜 그 달인지 월운+원국 관계로 설명 (월별 전체 나열 금지! 핵심 3개월만!)\n' +
      '④ 주의할 감정 시기: 갈등/이별 에너지가 강한 달 1~2개와 대처법\n' +
      '⑤ 솔로 vs 커플 맞춤 조언: 각각 2~3줄씩 구체적 행동 지침\n' +
      '모든 해석에 명리학적 근거 필수! 최소 15줄.\n\n' +
    ...
    const prompt2Yearly = promptBase +
      ...
      '[명리학 근거 필수] 모든 해석에 "왜?"를 명리학적으로 설명해. 예: "6월에 귀인을 만난다" → "6월 월운 갑오의 갑목(인성의 별)이 네 일간을 생해주는 구조라, 마치 비 온 뒤 무지개처럼 도움의 손길이 나타나는 달이야"\n' +
      ...
      '##13.올해 대인관계 & 귀인 캘린더##\n' +
      '[⚠️ 연애/로맨스 내용 절대 금지! 여기는 친구/동료/상사/멘토/가족 등 비연애 관계 전용. 연애는 섹션9에서 이미 다뤘어.]\n' +
      '올해 귀인이 나타나는 달, 어떤 띠/성격의 사람이 귀인인지. 갈등 조심할 달, 새로운 사회적 인연 들어오는 시기. 관계 에너지가 강한 달 vs 혼자 충전할 달.\n\n' +

      '##14.올해 나만의 행운 루틴##\n' +
      '용신 오행에 맞는 아침/저녁 습관, 요일별 운세 팁, 매일 실천할 개운 루틴 5가지. 구체적 시간/빈도 포함.\n\n' +

      '##15.올해 중요한 결정 타이밍 TOP 3##\n' +
      '계약/이직/결혼/이사 등 큰 결정을 하기 좋은 달 TOP 3. 각각 왜 그 달이 좋은지 세운+월운 합충형 근거로. 피해야 할 달도 1개.\n\n' +

      '##16.올해 하반기 반전 포인트##\n' +
      '상반기와 하반기 운세 흐름 차이, 하반기에 찾아오는 반전 계기. 하반기 집중 전략과 연말까지의 마무리 조언.\n\n' +

      '##17.2026 총평 & 마무리##\n' +
      '[이 섹션은 전체 올해운세의 마무리야. 앞 16개 섹션의 내용을 종합하되 반복하지 마!]\n' +
      '- 올해를 한 단어/문장으로 정의하면?\n' +
      '- 2025년(을사년)과 비교: 작년과 올해의 에너지 차이, 달라진 점을 구체적 명리 근거로\n' +
      '- 2027년(정미년) 미리보기: 올해가 내년으로 어떻게 이어지는지, 올해 뭘 준비하면 내년에 빛나는지\n' +
      '- 올해 인생 전체에서의 위치 (씨앗의 해? 수확의 해? 도약 준비의 해?)\n' +
      '- 나에게 보내는 따뜻한 한마디\n' +
      '최소 10줄. 감동적이고 비유를 많이 써서 마무리!\n\n' +
